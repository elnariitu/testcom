...
<body>
    ...
    <script src="games-data.js?v=2"></script>
    <script src="games-core.js?v=2"></script>
    <script src="games-a.js?v=2"></script>
    <script src="games-b.js?v=2"></script>
    <script>
        ...
        const GAME_ORDER = ['answer-rush', 'timeline-rush', 'history-map', 'who-am-i', 'true-or-trap', 'history-duel', 'capture-the-answer', 'history-millionaire'];
        ...
        function renderLobbyOnline(users) {
            lobbyOnlineCount.textContent = String(users.length);
            lobbyOnlineList.innerHTML = '';
            if (!users.length) {
                lobbyOnlineList.innerHTML = '<div class="lobby-online-empty">No users online</div>';
                return;
            }
            users.forEach(name => {
                ...
                row.querySelector('b').textContent = String(name).slice(0, 30);
                lobbyOnlineList.appendChild(row);
            });
        }

        async function refreshLobbyOnline() {
            if (lobbyPage.classList.contains('hidden')) return;
            try {
                const res = await fetch('auth.php?action=online_users', {
                    method: 'POST',
                    credentials: 'same-origin',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ nickname: lobbyMyName() })
                });
                const data = await res.json();
                renderLobbyOnline(Array.isArray(data.users) ? data.users : []);
            } catch (e) {
                lobbyOnlineList.innerHTML = '<div class="lobby-online-empty">Unavailable</div>';
            }
        }

        function startLobbyOnlineUpdates() {
            clearInterval(lobbyOnlineTimerId);
            refreshLobbyOnline();
            lobbyOnlineTimerId = setInterval(refreshLobbyOnline, 20000);
        }
